...
const allowedOrigins = ['https://www.polemicomtre.online', 'http://localhost:5174', 'https://polemicometre.xo.je'];
const corsOptions = {
...
app.post('/api/teams/delete-multiple', async (req, res) => { try { const { ids } = req.body; if (!ids || ids.length === 0) return res.status(400).json({ m: "Aucun ID fourni." }); await Team.deleteMany({ _id: { $in: ids } }); res.status(200).json({ m: "Équipes supprimées." }); } catch (e) { res.status(500).json({ m: e.message }); }});
...
app.post('/api/matches/batch', async (req, res) => { try { const { matches: matchesData, competitionId } = req.body; if (!matchesData || !Array.isArray(matchesData) || !competitionId) return res.status(400).json({ m: 'Données invalides' }); const newMatches = []; for (const matchInfo of matchesData) { const homeTeam = await Team.findOne({ name: matchInfo.home }); const awayTeam = await Team.findOne({ name: matchInfo.away }); if (homeTeam && awayTeam) { newMatches.push({ date_match: matchInfo.date, competition: competitionId, equipe_domicile: homeTeam._id, equipe_exterieur: awayTeam._id, status: 'à venir' }); } } await Match.insertMany(newMatches); res.status(201).json({ m: `${newMatches.length} matchs créés.` }); } catch (e) { res.status(500).json({ m: e.message }); }});
app.post('/api/matches/delete-multiple', async (req, res) => { try { const { ids } = req.body; if (!ids || ids.length === 0) return res.status(400).json({ m: "Aucun ID fourni." }); await Match.deleteMany({ _id: { $in: ids } }); res.status(200).json({ m: "Matchs supprimés." }); } catch (e) { res.status(500).json({ m: e.message }); }});
