...
// --- ROUTES POUR LES ACTIONS DE MASSE ---
...
app.post('/api/matches/delete-multiple', async (req, res) => {
    try {
        ...
        res.status(200).json({ m: "Matchs supprimés." });
    } catch (e) { res.status(500).json({ m: e.message }); }
});
app.post('/api/matches/batch', async (req, res) => {
    try {
        const { matches: matchesData, competitionId } = req.body;
        if (!matchesData || !Array.isArray(matchesData) || !competitionId) return res.status(400).json({ m: 'Données invalides' });
        const newMatches = [];
        for (const matchInfo of matchesData) {
            ...
            if (homeTeam && awayTeam) {
                newMatches.push({ date_match: matchInfo.date, competition: competitionId, equipe_domicile: homeTeam._id, equipe_exterieur: awayTeam._id, status: 'à venir' });
            }
        }
        await Match.insertMany(newMatches);
        res.status(201).json({ m: `${newMatches.length} matchs créés.` });
    } catch (e) { res.status(500).json({ m: e.message }); }
});
